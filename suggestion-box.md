...
- [ ] Two edge cases - diverted and switched tollfree.
  - The CIDVV platform should check for a vouch against the diverted number first. If that fails, then do the to number
  - I think vetting can identify switched tollfree. It means the vetting token should not expire in case you want to make two vetting calls (where the second is to the switched tollfree).
